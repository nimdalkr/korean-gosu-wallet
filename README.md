...
업비트·빗썸의 Base 메인넷 QUID 입금 주소군을 추적하는 비공개 온체인 리서치 데스크입니다. 각 주소의 30일 행동 성향·관심 자산·최근 변화·주요 상대방을 애널리스트 프로필로 만들고, 여러 지갑이 동시에 향하는 자산·프로토콜·섹터를 모아 조사할 알파 후보를 먼저 보여줍니다.
...
- 406개 지갑 각각의 운용 유형, 한 문단 애널리스트 뷰, 최근 행동 변화와 다음 확인 포인트
- Agency(직접 주도성), Depth(운용 깊이), Research Priority, 근거 신뢰도
- 지갑별 관심 자산·NFT·섹터, 30일 자산 유입·유출 장부, 14일 행동 리듬
- 직접 호출 프로토콜과 메서드, 반복 상대방, 중요 트랜잭션의 의미와 BaseScan 원문
- 코호트 전체에서 함께 부상하는 자산·프로토콜·섹터 테마와 운용 전략군
- 데일리 리서치 노트 형식의 핵심 결론, 우선 조사 지갑과 다음 24시간 확인 항목
...
신호와 지갑 점수는 가격 상승 확률이 아니라 조사 우선순위입니다. 지갑 리포트의 계산과 해석 원칙은 [`docs/wallet-research-model.md`](docs/wallet-research-model.md), 이벤트 신호의 점수 구성과 노이즈 방어는 [`docs/signal-model.md`](docs/signal-model.md)에 정리했습니다.
...
- QUID 입금은 코호트 선별 근거이므로 투자 성향·관심 자산·알파 테마 계산에서는 제외합니다.
- 단순 수신은 매수로 간주하지 않으며 “추정” 분류에는 항상 근거를 표시합니다.
- 무상 에어드롭과 스팸 NFT가 포함될 수 있으므로 단순 수신을 관심·보유 의도로 해석하면 안 됩니다.
- 섹터와 운용 유형은 최근 30일 행동의 규칙형 분류이며 프로젝트 공식 분류나 실소유자 신분을 확정하지 않습니다.
- USD 추정은 Blockscout 가격 메타데이터가 있는 transfer에 한정되며, 화면의 가격 커버리지를 함께 확인해야 합니다.
